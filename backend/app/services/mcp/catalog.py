...
CATALOG: tuple[ToolDefinition, ...] = (
    ...
    ToolDefinition("oyuns_tasks_search", "Search tasks", "Search tasks, blockers, and review work in the caller's permitted scope. To retrieve the caller's own tasks, pass the `employee_reference` given in the grounding context's `current_employee`. Omit `employee_reference` only to search the whole permitted scope (for example a manager's team view).", schemas.TasksSearchInput, "tasks", "read", intent_tags=frozenset({"tasks_read"})),
