...
function InfoSheet({ entry, onClose }: { entry: ContractArchiveEntry; onClose: () => void }) {
  ...
  return <div className="sheet-backdrop archive-sheet-backdrop" onMouseDown={onClose}><aside ref={sheetRef} className="archive-sheet" role="dialog" aria-modal="true" aria-labelledby="archive-info-title" onMouseDown={(event) => event.stopPropagation()}><header className="archive-sheet-header"><div><span className="eyebrow">МЭДЭЭЛЭЛ</span><h2 id="archive-info-title">{entry.name}</h2></div><button className="icon-button" onClick={onClose} aria-label="Хаах"><X size={19} /></button></header><div className="archive-info-grid"><span>Төрөл</span><strong>{entry.category}</strong><span>Хэмжээ</span><strong>{formatSize(entry.size)}</strong><span>Checksum</span><code>{entry.checksum}</code><span>Үүсгэсэн</span><strong>{formatDate(entry.created_at)}</strong><span>Хянасан</span><strong>{entry.reviewed_at ? formatDate(entry.reviewed_at) : 'Хүлээж байна'}</strong></div>{entry.can_edit && <section className="archive-expiry-settings"><h3>Хугацаа дуусах сануулга</h3><label>Дуусах огноо<input type="date" value={expiryOn} onChange={(event) => setExpiryOn(event.target.value)} /></label><fieldset><legend>Хуульчид урьдчилан сануулах</legend>{[1, 3, 7, 14, 30, 60, 90].map((days) => <label key={days}><input type="checkbox" checked={reminderDays.includes(days)} onChange={(event) => setReminderDays((current) => event.target.checked ? [...current, days].sort((a, b) => a - b) : current.filter((value) => value !== days))} />{days} хоногийн өмнө</label>)}</fieldset><button className="primary-action" disabled={save.isPending || detail.isLoading} onClick={saveExpiry}>{save.isPending && <LoaderCircle className="spin" size={15} />}Сануулга хадгалах</button></section>}<div className="archive-timeline"><h3>Үйл явдлын бүртгэл</h3>{detail.isLoading ? <LoaderCircle className="spin" size={20} /> : detail.data?.timeline?.length ? detail.data.timeline.map((event) => <div key={event.id}><strong>{event.operation}</strong><time>{formatDate(event.created_at)}</time></div>) : <p>Бүртгэл алга.</p>}</div></aside></div>
}
...
export function ContractArchiveWorkspace() {
  ...
  return <section className="contract-archive-workspace" onClick={() => actionId && setActionId(null)}>
    ...
    <section className="panel archive-browser" onDragOver={(event) => event.preventDefault()} onDrop={drop} aria-busy={data.isLoading || upload.isPending}><div className="archive-toolbar"><nav className="archive-breadcrumbs" aria-label="Гэрээний архивын зам"><button onClick={() => navigateFolder()}>Гэрээ</button><span>/</span><button onClick={() => navigateFolder()}>Архив</button>{data.data?.breadcrumbs.map((crumb) => <span key={crumb.id}> / <button onClick={() => navigateFolder(crumb.id)}>{crumb.name}</button></span>)}</nav><div className="archive-controls">{canManage && <button className="primary-action archive-icon-action" onClick={() => openCreateFolder()} aria-label="Шинэ хавтас үүсгэх" title="Шинэ хавтас үүсгэх"><FolderPlus size={16} /></button>}<label className="archive-search"><Search size={15} /><span className="sr-only">Архив хайх</span><input ref={searchRef} value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Хайх… /" /></label><DropdownSelect ariaLabel="Эрэмбэлэх" value={sort} onChange={(value) => setSort(value as typeof sort)} options={[{ value: 'name', label: 'Нэр' }, { value: 'created_at', label: 'Үүсгэсэн огноо' }, { value: 'size', label: 'Хэмжээ' }]} /><div className="archive-layout-toggle" aria-label="Харагдац"><button className={layout === 'list' ? 'active' : ''} onClick={() => chooseLayout('list')} aria-label="Жагсаалт"><List size={16} /></button><button className={layout === 'grid' ? 'active' : ''} onClick={() => chooseLayout('grid')} aria-label="Preview grid"><Grid2X2 size={16} /></button></div>{canManage && <button className="secondary-action archive-icon-action" onClick={() => uploadRef.current?.click()} aria-label="Файл нэмэх" title="Файл нэмэх"><Upload size={15} /></button>}<input ref={uploadRef} className="sr-only" type="file" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.tiff" onChange={(event) => { const file = event.target.files?.[0]; if (file) folderId ? upload.mutate({ folderId, category: 'Бусад', file }) : openCreateFolder(file); event.currentTarget.value = '' }} /></div></div>
