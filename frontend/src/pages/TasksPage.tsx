...
const STATUS_COLUMNS = [
  { key: 'open',        label: 'Открыто',    color: '#388BFD' },
  { key: 'in_progress', label: 'В работе',   color: '#D29922' },
  { key: 'done',        label: 'Выполнено',  color: '#3FB950' },
  { key: 'overdue',     label: 'Просрочено', color: '#F85149' },
] as const
    ...
    <div className="bg-surface2 border border-border rounded-xl p-3.5 mb-2 hover:border-accent transition-colors">
      ...
      {task.assignee_name && (
        <div className="text-xs text-muted mb-1.5 truncate">
          <span className="text-muted2">Исп:</span> {task.assignee_name}
        </div>
      )}
...
export function TasksPage() {
  ...
  return (
    <div>
      <PageHeader title="Задачи" sub="Канбан-доска">
        <Select
          value={filterAssignee}
          onChange={setFilterAssignee}
          options={employeeOptions}
        />
        <Btn variant="primary" onClick={() => setShowModal(true)}>+ Новая задача</Btn>
      </PageHeader>

      {tasksQuery.isLoading && (
        <div className="text-muted text-sm py-8 text-center">Загрузка...</div>
      )}
            ...
            return (
              <div key={col.key} className="flex flex-col">
                ...
                <div className="flex-1 bg-surface rounded-xl border border-border p-2.5 min-h-[120px]">
                  {tasks.length === 0 ? (
                    <div className="text-xs text-muted2 text-center py-4">Нет задач</div>
                  ) : (
                    tasks.map((task) => (
      ...
      {showModal && (
        <Modal title="Новая задача" onClose={() => { setShowModal(false); setForm(EMPTY_FORM) }}>
          <div className="flex flex-col gap-3.5">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs text-muted font-medium">Описание</label>
              <textarea
            ...
            />
            <div className="flex flex-col gap-1.5">
              <label className="text-xs text-muted font-medium">Дедлайн</label>
              <input
                type="datetime-local"
            ...
            <div className="flex gap-2.5 justify-end pt-1">
              <Btn onClick={() => { setShowModal(false); setForm(EMPTY_FORM) }}>Отмена</Btn>
              <Btn
                variant="primary"
