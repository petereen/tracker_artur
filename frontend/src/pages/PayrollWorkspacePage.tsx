...
import { PayrollAnnualChart, PayrollHistoryTable, PayrollMetricCard, PayrollPayslipTable, PayrollShortcutGrid, PayrollWorkspaceTabs, buildPayrollTrend, payrollPercentDelta } from '../components/payroll/PayrollWorkspaceUI'
...
function FrappeWorkspaceShell({ title, subtitle, children, actions, showTabs = true }: { title: string; subtitle: string; children: ReactNode; actions?: ReactNode; showTabs?: boolean }) {
...
function PayrollWorkspaceDashboard({ onCreate }: { onCreate: () => void }) {
  ...
  const activeEntries = entries.data?.filter((entry) => (entry.document_status || entry.status) !== 'cancelled') || []
  const orderedEntries = [...activeEntries].sort((a, b) => String(b.period_end).localeCompare(String(a.period_end)))
  const latest = orderedEntries[0]
  const previous = latest ? orderedEntries.find((entry) => entry.id !== latest.id && String(entry.period_end).slice(0, 7) !== String(latest.period_end).slice(0, 7)) : undefined
  const openEntries = activeEntries.filter((entry) => entry.document_status !== 'submitted').length
  const pendingBank = activeEntries.filter((entry) => entry.salary_slips_submitted && !entry.bank_entry_id).length
  const latestDeductions = Number(latest?.total_pit || 0) + Number(latest?.total_employee_shi || 0)
  const delta = (current: number, prior?: string) => { const value = payrollPercentDelta(current, prior === undefined ? null : Number(prior)); return value === null ? 'Өмнөх үеийн харьцуулалт алга' : `${value >= 0 ? '+' : ''}${value.toFixed(1)}% өмнөх үеэс` }
  const cards = [
    { label: 'Gross salary', value: formatPayrollMoney(latest?.total_gross || '0'), detail: delta(Number(latest?.total_gross || 0), previous?.total_gross), icon: WalletCards, tone: 'blue' as const },
    { label: 'Net salary', value: formatPayrollMoney(latest?.total_net || '0'), detail: delta(Number(latest?.total_net || 0), previous?.total_net), icon: WalletCards, tone: 'green' as const },
    { label: 'PIT + SHI', value: formatPayrollMoney(String(latestDeductions)), detail: latest ? `${latest.run_number} · employee deductions` : 'Тооцсон deduction алга', icon: Calculator, tone: 'orange' as const },
    { label: 'Банкны шилжүүлэг', value: String(pendingBank), detail: 'Баталгаажуулахыг хүлээж буй', icon: Landmark, tone: 'purple' as const },
  ]
  const groups = [
  ...
  return <FrappeWorkspaceShell title="Цалин бодолт" subtitle="Mongolia payroll · Frappe-style баримтын ажлын орчин" actions={<button className="primary-action" onClick={onCreate}><Plus size={16} />Шинэ Payroll Entry</button>}>
    <section className="payroll-reference-summary"><div className="payroll-reference-kpi-grid">{cards.map((card) => <PayrollMetricCard {...card} key={card.label} />)}</div><PayrollAnnualChart data={buildPayrollTrend(entries.data || [])} /></section>
    <section className="panel payroll-reference-history"><div className="view-toolbar"><div><span className="eyebrow">PAYROLL HISTORY</span><h2>Сүүлийн Payroll Entries</h2><p>Цуцлагдаагүй бичилтийн gross, net pay болон төлөв.</p></div><Link className="secondary-action" to="/erp/payroll/payroll-entries">Бүгдийг харах</Link></div>{entries.isLoading ? <p>Уншиж байна…</p> : entries.isError ? <div className="payroll-inline-error"><CircleAlert size={16} />Payroll Entry жагсаалт ачааллахад алдаа гарлаа.</div> : activeEntries.length ? <PayrollHistoryTable entries={orderedEntries.slice(0, 8)} statusLabel={payrollDocumentStatusLabel} formatMoney={formatPayrollMoney} /> : <EmptyState title="Payroll Entry алга" copy="Сарын цалингийн бодолтыг эхлүүлэхийн тулд шинэ баримт үүсгэнэ үү." action={<button className="secondary-action" onClick={onCreate}><Plus size={14} />Шинэ баримт</button>} />}</section>
    <PayrollShortcutGrid groups={groups.map(({ title, icon, links }) => ({ title, icon, links: links.map(([en, mn, href]) => ({ label: mn, meta: en, href })) }))} />
    <section className="panel frappe-payroll-table-card payroll-reference-readiness"><div className="view-toolbar"><div><span className="eyebrow">SETUP READINESS</span><h2>Тохиргооны бэлэн байдал</h2></div><CalendarDays size={19} /></div><div className="frappe-payroll-readiness"><div><span>Payroll Periods</span><strong>{periods.data?.length || 0}</strong></div><div><span>Salary Components</span><strong>{components.data?.length || 0}</strong></div><div><span>Ажилтнууд</span><strong>{workers.data?.length || 0}</strong></div><div><span>Salary Slips</span><strong>{slips.data?.length || 0}</strong></div></div><p className="payroll-helper"><ShieldCheck size={15} />НДШ, ХХОАТ болон encrypted банкны дансны тохиргоог нэг Payroll Entry урсгал ашиглана.</p></section>
  </FrappeWorkspaceShell>
}

function PayrollEmployeeSelfServiceView() {
  const payslips = useMyPayrollPayslips(true)
  return <FrappeWorkspaceShell title="Миний цалин" subtitle="Нийтлэгдсэн Salary Slip-ээ харах, хамгаалагдсан PDF-ээр татах орон зай." showTabs={false}>
