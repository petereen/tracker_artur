...
export function EnterpriseReportsPage() {
  ...
  const [createForm, setCreateForm] = useState<{ report_type: 'daily' | 'monthly' | 'next_month_plan'; period_date: string }>({ report_type: 'daily', period_date: new Date().toISOString().slice(0, 10) })
  ...
  return <div>
    <div className="view-toolbar"><div><h2>{canReview ? 'Багийн тайлан' : 'Миний тайлан'}</h2><p>{canReview ? 'Ажилтны илгээсэн тайланг хянаж, засвар хүсэх эсвэл батална.' : 'Ноорог хадгалж, бэлэн болсон үед нэг удаа илгээнэ.'}</p></div><div className="toolbar-cluster"><TimePeriodFilter preset={periodPreset} period={period} onChange={(nextPreset, nextPeriod) => { setPeriodPreset(nextPreset); setPeriod(nextPeriod) }} />{!canReview && <button className="primary-action compact" onClick={() => setCreating(true)}><Plus size={16} />Тайлан үүсгэх</button>}<div className="report-summary"><span><strong>{reports.data?.filter((report) => report.status === 'submitted').length ?? 0}</strong> хүлээгдэж буй</span><span><strong>{reports.data?.filter((report) => report.status === 'approved').length ?? 0}</strong> батлагдсан</span></div></div></div>
    ...
    <AnimatePresence>{creating && <motion.div className="sheet-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={() => setCreating(false)}><motion.aside className="detail-sheet" initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} onMouseDown={(event) => event.stopPropagation()}><div className="sheet-header"><div><span className="eyebrow">New report</span><h2>Тайлан үүсгэх</h2></div><button onClick={() => setCreating(false)} aria-label="Хаах"><X /></button></div><form className="sheet-form" onSubmit={async (event) => { event.preventDefault(); const result = await createReport.mutateAsync(createForm); setCreating(false); setSelectedId(result.id) }}><label>Тайлангийн төрөл<select value={createForm.report_type} onChange={(event) => setCreateForm({ ...createForm, report_type: event.target.value as typeof createForm.report_type })}><option value="daily">Өдрийн тайлан</option><option value="monthly">Сарын тайлан</option><option value="next_month_plan">Дараа сарын төлөвлөгөө</option></select></label><label>Хугацаа<input type="date" required value={createForm.period_date} onChange={(event) => setCreateForm({ ...createForm, period_date: event.target.value })} /></label><button className="primary-action" disabled={createReport.isPending}>Үүсгэх</button></form></motion.aside></motion.div>}</AnimatePresence>
  </div>
}
