...
import { Check, Download, FileCheck2, MessageSquareWarning, Paperclip, Plus, RotateCcw, Save, Send, Trash2, X } from 'lucide-react'
import { EMPTY_ROLES, useAuthStore } from '../store/auth'
import { downloadAttachment, useAddReportComment, useAttachments, useBatchApproveReports, useCreateReport, useDeleteAttachment, useEnterpriseReports, useReportDetail, useReportReview, useResolveReportComment, useSaveReportDraft, useUploadAttachment } from '../api/enterprise'
...
function ReportReviewWorkspace({ reportId, detail, canReview }: { reportId: number; detail: any; canReview: boolean }) {
  ...
  return <section className="report-collaboration"><h3>Хувилбар харьцуулах</h3><div className="revision-selectors"><select value={leftId ?? ''} onChange={(e) => setLeftId(Number(e.target.value))}>{revisions.map((item: any) => <option key={item.id} value={item.id}>#{item.id} · {item.status}</option>)}</select><select value={rightId ?? ''} onChange={(e) => setRightId(Number(e.target.value))}>{revisions.map((item: any) => <option key={item.id} value={item.id}>#{item.id} · {item.status}</option>)}</select></div><div className="revision-compare"><article><ReactMarkdown remarkPlugins={[remarkGfm]}>{left?.markdown ?? ''}</ReactMarkdown></article><article><ReactMarkdown remarkPlugins={[remarkGfm]}>{right?.markdown ?? ''}</ReactMarkdown></article></div>
...
export function EnterpriseReportsPage() {
  ...
  return <div>
    {canReview && <div className="batch-review panel"><label>Багцаар батлах<select multiple value={batchIds.map(String)} onChange={(event) => setBatchIds(Array.from(event.currentTarget.selectedOptions, (option) => Number(option.value)))}>{reports.data?.filter((report) => report.report_type === 'monthly' && report.status === 'submitted').map((report) => <option key={report.id} value={report.id}>{report.employee_name} · {report.period_date}</option>)}</select></label><button className="primary-action compact" disabled={!batchIds.length || batchApprove.isPending} onClick={() => batchApprove.mutate(batchIds)}>Сонгосныг батлах</button></div>}
