...
import { Check, Download, FileArchive, FileCheck2, Paperclip, Plus, RotateCcw, Save, Send, Trash2, X } from 'lucide-react'
...
import { QueryRegion, Skeleton, TableSkeleton, toQueryRegionState } from '../components/Loading'
...
export function EnterpriseReportsPage() {
  ...
  // Deep link from notifications and shared chat cards: /reports?report=ID
  const [selectedId, setSelectedIdState] = useState<number | undefined>(() => Number(searchParams.get('report')) || undefined)
  const setSelectedId = (id?: number) => {
    setSelectedIdState(id)
    if (!id && searchParams.has('report')) { const next = new URLSearchParams(searchParams); next.delete('report'); setSearchParams(next, { replace: true }) }
  }
  useEffect(() => { const linked = Number(searchParams.get('report')) || undefined; if (linked) setSelectedIdState(linked) }, [searchParams])
  ...
  const canExport = roles.some((role) => ['admin', 'manager'].includes(role))
