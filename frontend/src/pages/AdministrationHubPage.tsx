...
import { useBrandingSettings, useCreateManagedAccount, useGoogleCalendarConnect, useManagedAccounts, usePermissionSettings, useUpdateBrandingSettings, useUpdateManagedAccount, useUpdatePermissionSettings, useUploadBrandingLogo } from '../api/enterprise'
...
function BrandingSettingsPanel() {
  ...
  const select = (theme: 'light' | 'dark', source: string) => {
    ...
    update.mutate({ theme, source: source as 'legacy-aio' | 'legacy-icon' | 'default' })
  }
  const uploadFile = (theme: 'light' | 'dark', file?: File) => { if (file) upload.mutate({ theme, file }) }
  return <section className="branding-settings panel"><div className="panel-heading"><div><span className="eyebrow">Workspace identity</span><h2>Лого ба theme</h2><p>Light болон dark горимд тусдаа лого сонгох эсвэл шинэ зураг байршуулна.</p></div><Settings2 /></div><div className="branding-grid">{(['light', 'dark'] as const).map((theme) => { const logo = theme === 'light' ? branding.data?.light_logo : branding.data?.dark_logo; return <article className={`branding-card ${theme}`} key={theme}><div className="branding-preview"><img src={logo || '/favicon.png'} alt={`${theme} logo preview`} /></div><div className="branding-card-body"><strong>{theme === 'light' ? 'Light mode' : 'Dark mode'}</strong><select aria-label={`${theme} logo сонгох`} value={sourceFor(theme)} onChange={(event) => select(theme, event.target.value)} disabled={branding.isLoading || update.isPending}><option value="default">Автомат legacy</option>{branding.data?.legacy_options.map((option) => <option value={option.value} key={option.value}>{option.label}</option>)}{sourceFor(theme) === 'uploaded' && <option value="uploaded">Uploaded logo</option>}</select><label className="secondary-action compact branding-upload"><span>Шинэ зураг сонгох</span><input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => { uploadFile(theme, event.target.files?.[0]); event.currentTarget.value = '' }} disabled={upload.isPending} /></label></div></article> })}</div></section>
}
