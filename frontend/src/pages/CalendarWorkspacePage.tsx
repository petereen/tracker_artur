...
function itemDates(item: any) {
  ...
  const start = item.start_at || item.starts_at || item.deadline_at || item.ends_at
  const end = item.kind === 'task' ? item.deadline_at : (item.ends_at || item.deadline_at || item.start_at || item.starts_at)
...
export function CalendarWorkspacePage() {
  ...
  return <div className="calendar-workspace"><div className="view-toolbar"><div><h2>Календарь</h2><p>Даалгавар, сануулга, үйл явдал болон компаний чухал өдрүүд.</p></div><div className="toolbar-cluster"><div className="segmented-control"><button className={scope === 'private' ? 'active' : ''} onClick={() => startTransition(() => setScope('private'))}>Хувийн</button><button className={scope === 'corporate' ? 'active' : ''} onClick={() => startTransition(() => setScope('corporate'))}>Компаний</button></div><button className="primary-action compact" onClick={() => { setForm({ ...form, visibility: scope === 'corporate' && canPublish ? 'company' : 'private' }); setCreating(true) }}><Plus size={16} />Үүсгэх</button></div></div>
    ...
    <QueryRegion pending={events.isLoading || events.isFetching} skeleton={<CalendarSkeleton />}><div className="planning-calendar panel">{days.map((day) => { const key = localDate(day); const items = all.filter((item: any) => itemDates(item).includes(key)); const redDay = day.getDay() === 0 || day.getDay() === 6 || holidayKeys.has(key); return <section key={key} className={`${day.getMonth() === anchor.getMonth() ? '' : 'outside'} ${redDay ? 'red-day' : ''} ${key === todayKey ? 'today' : ''}`}><header><strong>{day.getDate()}</strong><span>{day.toLocaleDateString('mn-MN', { weekday: 'short' })}</span></header>{items.map((item: any) => { const dates = itemDates(item); const position = dates.length > 1 ? (dates[0] === key ? 'range-start' : dates[dates.length - 1] === key ? 'range-end' : 'range-middle') : ''; return <button className={`calendar-item ${item.kind} ${position}`} key={`${item.kind}-${item.id}`} onClick={() => setSelected(item)}><strong>{item.title}</strong><small>{item.kind === 'task' ? item.primary_owner_name || 'Даалгавар' : item.kind === 'holiday' ? 'Нийтийн амралт' : item.kind === 'birthday' ? 'Төрсөн өдөр' : item.visibility === 'company' ? 'Компаний үйл явдал' : item.kind === 'reminder' ? 'Сануулга' : 'Хувийн үйл явдал'}</small></button>})}</section> })}</div></QueryRegion>
    <AnimatePresence>{selected && <motion.div className="sheet-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={() => setSelected(null)}><motion.aside className="detail-sheet" initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', bounce: 0, duration: .4 }} onMouseDown={(event) => event.stopPropagation()}><div className="sheet-header"><div><span className="eyebrow">Calendar item</span><h2>{selected.title}</h2></div><button className="sheet-close" onClick={() => setSelected(null)} aria-label="Хаах">×</button></div><div className="calendar-detail"><p className="calendar-detail-type">{selected.kind === 'task' ? 'Даалгавар' : selected.kind === 'holiday' ? 'Нийтийн амралт' : selected.kind === 'birthday' ? 'Төрсөн өдөр' : selected.kind === 'reminder' ? 'Сануулга' : 'Үйл явдал'}</p>{selected.description && <p>{selected.description}</p>}<dl><div><dt>Эхлэх</dt><dd>{calendarDate(selected.start_at || selected.starts_at || selected.holiday_date) || '—'}</dd></div><div><dt>Дуусах</dt><dd>{calendarDate(selected.deadline_at || selected.ends_at) || '—'}</dd></div>{selected.kind === 'task' && <><div><dt>Төлөв</dt><dd>{selected.workflow_status || '—'}</dd></div><div><dt>Хариуцагч</dt><dd>{selected.primary_owner_name || 'Даалгавар'}</dd></div>{selected.project_name && <div><dt>Төсөл</dt><dd>{selected.project_name}</dd></div>}</>}</dl></div></motion.aside></motion.div>}</AnimatePresence>
