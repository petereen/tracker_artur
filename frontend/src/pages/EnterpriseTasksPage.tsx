...
import { CalendarDays, CheckCircle2, Circle, Clock3, GripVertical, LayoutGrid, List, Plus, Rows3, X } from 'lucide-react'
...
export function EnterpriseTasksPage() {
  ...
  const tasks = useEnterpriseTasks(projectId)
  ...
  const [form, setForm] = useState({ title: '', description: '', project_id: projectId ? String(projectId) : '', workflow_status: 'to_do', deadline_at: '', estimate_minutes: '' })
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    await createTask.mutateAsync({ title: form.title, description: form.description || null, project_id: form.project_id ? Number(form.project_id) : null, workflow_status: form.workflow_status, deadline_at: form.deadline_at ? new Date(form.deadline_at).toISOString() : null, estimate_minutes: form.estimate_minutes ? Number(form.estimate_minutes) : null })
    setCreating(false); setForm({ title: '', description: '', project_id: projectId ? String(projectId) : '', workflow_status: 'to_do', deadline_at: '', estimate_minutes: '' })
  }
  return <div className="task-workspace"><div className="view-toolbar"><div><h2>Ажлын урсгал</h2><p>{projectId ? projects.data?.find((project) => project.id === projectId)?.name : 'Бүх төслийн даалгавар'}</p></div><div className="toolbar-cluster"><div className="segmented-control" aria-label="Харагдац"><button className={view === 'board' ? 'active' : ''} onClick={() => setView('board')}><LayoutGrid size={15} />Самбар</button><button className={view === 'list' ? 'active' : ''} onClick={() => setView('list')}><List size={15} />Жагсаалт</button><button className={view === 'timeline' ? 'active' : ''} onClick={() => setView('timeline')}><Rows3 size={15} />Timeline</button></div><button className="primary-action compact" onClick={() => setCreating(true)}><Plus size={16} />Даалгавар</button></div></div>
    ...
    {view === 'timeline' && <div className="timeline-view panel"><div className="timeline-axis"><span>Өнгөрсөн</span><span>Өнөөдөр</span><span>Ирэх 7 хоног</span><span>Дараа</span></div>{(tasks.data ?? []).map((task, index) => <button key={task.id} onClick={() => setSelected(task)}><strong>{task.title}</strong><span style={{ left: `${Math.min(76, 12 + index * 7)}%`, width: `${Math.max(8, Math.min(30, (task.estimate_minutes ?? 120) / 30))}%` }} /></button>)}</div>}
    <AnimatePresence>{(selected || creating) && <motion.div className="sheet-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={() => { setSelected(null); setCreating(false) }}><motion.aside className="detail-sheet" initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', bounce: 0, duration: .4 }} onMouseDown={(event) => event.stopPropagation()}>{selected ? <><div className="sheet-header"><div><span className="eyebrow">Task #{selected.id}</span><h2>{selected.title}</h2></div><button onClick={() => setSelected(null)}><X /></button></div><div className="task-detail-meta"><span>{COLUMNS.find((column) => column.key === selected.workflow_status)?.label}</span><span>Priority {selected.priority}</span><span>v{selected.version}</span></div><div className="markdown-content"><ReactMarkdown>{selected.description || '_Тайлбар оруулаагүй байна._'}</ReactMarkdown></div><div className="detail-section"><h3>Гүйцэтгэл</h3><div className="progress-track"><span style={{ width: `${selected.workflow_status === 'done' ? 100 : COLUMNS.findIndex((column) => column.key === selected.workflow_status) * 22}%` }} /></div></div></> : <><div className="sheet-header"><div><span className="eyebrow">Quick create</span><h2>Шинэ даалгавар</h2></div><button onClick={() => setCreating(false)}><X /></button></div><form className="sheet-form" onSubmit={submit}><label>Гарчиг<input value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} autoFocus required /></label><label>Тайлбар (Markdown)<textarea rows={6} value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></label><label>Төсөл<select value={form.project_id} onChange={(event) => setForm({ ...form, project_id: event.target.value })}><option value="">Төсөлгүй</option>{projects.data?.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select></label><div className="form-row"><label>Хугацаа<input type="datetime-local" value={form.deadline_at} onChange={(event) => setForm({ ...form, deadline_at: event.target.value })} /></label><label>Тооцоолсон минут<input type="number" min="0" value={form.estimate_minutes} onChange={(event) => setForm({ ...form, estimate_minutes: event.target.value })} /></label></div><button className="primary-action" disabled={createTask.isPending}>Үүсгэх</button></form></>}</motion.aside></motion.div>}</AnimatePresence></div>
}
