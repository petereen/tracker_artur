  ...
  return <div>
    ...
    <Card className="admin-table-card !p-0 overflow-hidden">
      <div className="px-5 py-3.5 border-b border-border flex flex-wrap gap-3 items-center">
        ...
        <select value={employeeId} onChange={(e) => setEmployeeId(e.target.value)} className="bg-surface2 border border-border rounded-lg px-3 py-[7px] text-[13px] outline-none"><option value="">Бүх ажилтан</option>{employees.map((employee: any) => <option key={employee.id} value={employee.id}>{employee.name}</option>)}</select>
        ...
        <select value={status} onChange={(e) => setStatus(e.target.value)} className="bg-surface2 border border-border rounded-lg px-3 py-[7px] text-[13px] outline-none"><option value="">Бүх төлөв</option>{Object.entries(STATUS_LABELS).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select>
      </div>
