...
function explain(row: MonthlyPayrollRunRow, month: MonthlyPayrollMonth, isFinal: boolean): Explanation[] {
  ...
  const advanceLine: Explanation[] = isFinal ? [] : [{
    ...
    formula: basis === 'PERCENT' ? 'Үндсэн цалин × хувь' : basis === 'FIXED' ? 'HR-ийн оруулсан дүн' : 'Таслах өдөр хүртэл олсон цалин + (хоол + унаа) × өдөр − өмнө батлагдсан урьдчилгаа',
  ...
  const lines: Explanation[] = [
    ...
    { label: 'Суутгалын дүн', manual: false, formula: isFinal ? 'Урьдчилгаа + ХХОАТ + НДШ + бусад суутгал' : 'Урьдчилгаа + ХХОАТ + НДШ + хоол унаа + бусад суутгал', inputs: isFinal ? `${formatAmount(result.advance)} + ${formatAmount(result.pit)} + ${formatAmount(result.employee_shi)} + ${formatAmount(result.other_deductions)}` : `${formatAmount(result.advance)} + ${formatAmount(result.pit)} + ${formatAmount(result.employee_shi)} + ${formatAmount(result.meal_commute)} + ${formatAmount(result.other_deductions)}`, value: formatAmount(result.total_deductions) },
