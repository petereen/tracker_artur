...
export function RowDrawer({ row, run, month, editable, onClose, askReason }: {
  ...
  useEffect(() => { setLines((row.inputs.other_deductions || []).map((line: any) => ({ type: line.type || '', amount: String(line.amount || ''), note: line.note || '' }))) }, [row.inputs.other_deductions])
