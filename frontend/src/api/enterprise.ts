...
export interface PayrollPaymentAllocation { id: number; payslip_id: number; employee_id: number; amount: string; status: 'pending' | 'bank_submitted' | 'settled' | 'rejected' | 'reversed'; attempt_number: number; transaction_reference?: string | null; settled_at?: string | null; rejection_reason?: string | null }
...
export function usePayrollSalaryRegister(runId?: number, enabled = true) { return useQuery<Record<string, unknown>>({ queryKey: [...frappePayrollQueryKey, 'reports', 'salary-register', runId], queryFn: () => api.get('/v1/erp/payroll/reports/salary-register', { params: { run_id: runId } }).then((r) => r.data), enabled, retry: false }) }
...
export function usePayrollBankRemittance(runId?: number, enabled = true) { return useQuery<Record<string, unknown>>({ queryKey: [...frappePayrollQueryKey, 'reports', 'bank-remittance', runId], queryFn: () => api.get('/v1/erp/payroll/reports/bank-remittance', { params: { run_id: runId } }).then((r) => r.data), enabled, retry: false }) }
...
export function useCreatePayrollRun() { const qc = useQueryClient(); return useMutation({ mutationFn: (input: PayrollEntryInput) => api.post('/v1/erp/payroll/runs', input).then((r) => r.data as PayrollRun), onSuccess: () => qc.invalidateQueries({ queryKey: ['v1', 'erp', 'payroll', 'runs'] }) }) }
export function useReplacePayrollRun() { const qc = useQueryClient(); return useMutation({ mutationFn: ({ id, input }: { id: number; input: PayrollEntryInput }) => api.post(`/v1/erp/payroll/runs/${id}/replace`, input).then((r) => r.data as PayrollRun), onSuccess: () => qc.invalidateQueries({ queryKey: ['v1', 'erp', 'payroll'] }) }) }
...
export function usePreparePayrollPayments() { const qc = useQueryClient(); return useMutation({ mutationFn: ({ id, ...input }: { id: number; payment_account_id?: number; posting_date?: string; allocations?: Array<{ payslip_id: number; amount: string }>; retry_of_batch_id?: number }) => api.post(`/v1/erp/payroll/runs/${id}/payments`, input).then((r) => r.data as PayrollPaymentBatch), onSuccess: () => qc.invalidateQueries({ queryKey: ['v1', 'erp', 'payroll'] }) }) }
export function usePayrollPaymentBatch(id: number | undefined, enabled = true) { return useQuery<PayrollPaymentBatch>({ queryKey: ['v1', 'erp', 'payroll', 'payments', id], queryFn: () => api.get(`/v1/erp/payroll/payments/${id}`).then((r) => r.data), enabled: enabled && Boolean(id), retry: false }) }
export function usePayrollPaymentBatches(runId?: number, enabled = true) { return useQuery<PayrollPaymentBatch[]>({ queryKey: ['v1', 'erp', 'payroll', 'payments', runId], queryFn: () => api.get('/v1/erp/payroll/payments', { params: { run_id: runId } }).then((r) => r.data), enabled, retry: false }) }
