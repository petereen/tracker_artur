...
export type PayrollEntryInput = { run_type: PayrollRun['run_type']; payroll_period_id?: number | null; period_start: string; period_end: string; posting_date?: string; tax_point_date: string; payroll_frequency?: 'monthly'; statutory_profile_id?: number; employee_ids?: number[]; employee_filter?: Record<string, string | number | null>; cost_center_id?: number | null; payment_account_id?: number | null; validate_attendance?: boolean; input_overrides?: Record<string, Record<string, unknown>>; variable_inputs?: PayrollVariableInput[] }
...
export function usePayrollReport(id: number | undefined, kind: 'nd7' | 'nd7a' | 'nd7b' | 'nd8' | 'tt11', enabled = true) { return useQuery<Record<string, unknown>>({ queryKey: ['v1', 'erp', 'payroll', 'reports', id, kind], queryFn: () => api.get(`/v1/erp/payroll/runs/${id}/reports/${kind}`).then((r) => r.data), enabled: enabled && Boolean(id), retry: false }) }
...
export async function downloadPayrollReport(id: number, kind: 'nd7' | 'nd7a' | 'nd7b' | 'nd8' | 'tt11', format: 'csv' | 'xlsx' | 'pdf' = 'csv') { const response = await api.get(`/v1/erp/payroll/runs/${id}/reports/${kind}`, { params: { format }, responseType: 'blob' }); saveCompanyBlob(response.data, `${kind}-${id}.${format}`) }
