...
export function usePayrollRun(id: number | undefined, enabled = true) { return useQuery<PayrollRun & { payslips: PayrollPayslip[] }>({ queryKey: ['v1', 'erp', 'payroll', 'runs', id], queryFn: () => api.get(`/v1/erp/payroll/runs/${id}`).then((r) => r.data), enabled: enabled && Boolean(id), retry: false }) }
...
export function useApprovePayrollRun() { const qc = useQueryClient(); return useMutation({ mutationFn: (id: number) => api.post(`/v1/erp/payroll/runs/${id}/approve`).then((r) => r.data as PayrollRun), onSuccess: () => qc.invalidateQueries({ queryKey: ['v1', 'erp', 'payroll'] }) }) }
