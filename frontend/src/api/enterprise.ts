...
export interface PayrollSHIRate { payer: 'employee' | 'employer'; insurance_fund: string; insured_category: string; hazard_class: string; rate: string; base_floor: string; exemption_code: string | null }
export interface PayrollPITBracket { period_basis: 'monthly' | 'annual' | 'period'; lower_bound: string; upper_bound: string | null; marginal_rate: string; base_tax: string }
export interface PayrollReliefTier { eligibility_code: string; lower_bound: string; upper_bound: string | null; fixed_amount: string; amount_basis: 'monthly' | 'annual' | 'period'; formula: string | null }
export interface PayrollProfile { id: number; code: string; jurisdiction?: string; version: number; status: string; effective_from: string; effective_to: string | null; tax_point_basis?: string; currency?: string; minimum_wage: string; shi_ceiling_multiplier: string; pit_withholding_method: string; rounding_policy?: Record<string, unknown>; leave_policy?: Record<string, unknown>; source_references?: string[]; is_example: boolean; checksum: string; shi_rates?: PayrollSHIRate[]; pit_brackets?: PayrollPITBracket[]; relief_tiers?: PayrollReliefTier[] }
...
export function usePayrollReport(id: number | undefined, kind: 'nd7' | 'nd8' | 'tt11', enabled = true) { return useQuery<Record<string, unknown>>({ queryKey: ['v1', 'erp', 'payroll', 'reports', id, kind], queryFn: () => api.get(`/v1/erp/payroll/runs/${id}/reports/${kind}`).then((r) => r.data), enabled: enabled && Boolean(id), retry: false }) }
...
export async function downloadPayrollReport(id: number, kind: 'nd7' | 'nd8' | 'tt11', format: 'csv' | 'xlsx' = 'csv') { const response = await api.get(`/v1/erp/payroll/runs/${id}/reports/${kind}`, { params: { format }, responseType: 'blob' }); saveCompanyBlob(response.data, `${kind}-${id}.${format}`) }
