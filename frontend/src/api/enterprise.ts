...
export function useUpdateHREmployee() { const qc = useQueryClient(); return useMutation({ mutationFn: ({ id, ...input }: { id: number } & Record<string, unknown>) => api.patch(`/v1/hr/employees/${id}`, input).then((r) => r.data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['v1', 'hr'] }); qc.invalidateQueries({ queryKey: ['employees'] }) } }) }
...
export interface PayrollStructureAssignment { id: number; employee_id: number; salary_structure_id: number; effective_from: string; effective_to: string | null; base_salary: string; insured_category: string; hazard_class: string; payment_method: string; document_status?: 'draft' | 'submitted' | 'cancelled' }
