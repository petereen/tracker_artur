...
export interface PayrollCapabilities { capabilities: Record<'view' | 'view_salary' | 'create' | 'administer' | 'edit_setup' | 'calculate' | 'approve' | 'review' | 'post' | 'pay' | 'release_slips' | string, boolean> }
...
export interface PayrollWorkPolicy { id: number; code: string; name: string; scope_type: 'organization' | 'job_title' | 'employee'; scope_key: string; effective_from: string; effective_to?: string | null; daily_hours: string; weekly_hours: string; workweek: number[]; overtime_rules: Record<string, unknown>; stacking_policy: 'exclusive' | 'stack'; status: string; source_references: string[] }
...
export function usePayrollSalaryRegister(runId?: number, enabled = true) { return useQuery<Array<{ salary_slip_id: number; employee_id: number; gross: string; net_pay: string; status: string }>>({ queryKey: [...frappePayrollQueryKey, 'reports', 'salary-register', runId], queryFn: () => api.get('/v1/erp/payroll/reports/salary-register', { params: { run_id: runId } }).then((r) => r.data), enabled, retry: false }) }
