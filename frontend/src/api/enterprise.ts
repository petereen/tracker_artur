...
export function useUpdateHRLeave() { const qc = useQueryClient(); return useMutation({ mutationFn: ({ id, ...input }: { id: number; leave_type?: HRLeaveRequest['leave_type']; starts_on?: string; ends_on?: string; reason?: string; version?: number }) => api.patch(`/v1/hr/leave-requests/${id}`, input).then((r) => r.data as HRLeaveRequest), onSuccess: () => qc.invalidateQueries({ queryKey: ['v1', 'hr'] }) }) }
