...
export interface PayrollProfile { id: number; code: string; version: number; status: string; effective_from: string; effective_to: string | null; minimum_wage: string; shi_ceiling_multiplier: string; pit_withholding_method: string; is_example: boolean; checksum: string }
...
export function usePayrollStructures(enabled = true) { return useQuery<Array<{ id: number; code: string; name: string; version: number; status: string; effective_from: string; effective_to: string | null; currency: string; checksum: string; components: Array<{ code: string; name: string; component_kind: string; formula: string; proration_basis: string; is_taxable: boolean; is_shi_subject: boolean; is_non_taxable_allowance: boolean; account_id: number | null; cost_center_id: number | null }> }>>({ queryKey: ['v1', 'erp', 'payroll', 'salary-structures'], queryFn: () => api.get('/v1/erp/payroll/salary-structures').then((r) => r.data), enabled, retry: false }) }
