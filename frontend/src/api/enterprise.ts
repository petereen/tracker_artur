...
export function usePayrollBankAccounts(employeeId?: number, enabled = true) { return useQuery<PayrollBankAccount[]>({ queryKey: ['v1', 'erp', 'payroll', 'employees', employeeId, 'bank-accounts'], queryFn: () => api.get(`/v1/erp/payroll/employees/${employeeId}/bank-accounts`).then((r) => r.data), enabled: enabled && Boolean(employeeId), retry: false }) }
export function useSavePayrollBankAccount() { const qc = useQueryClient(); return useMutation({ mutationFn: ({ employeeId, ...input }: { employeeId: number; bank_code: string; account_number: string; account_holder?: string; is_primary?: boolean; valid_from: string; valid_to?: string | null }) => api.post(`/v1/erp/payroll/employees/${employeeId}/bank-account`, input).then((r) => r.data as PayrollBankAccount), onSuccess: () => qc.invalidateQueries({ queryKey: ['v1', 'erp', 'payroll'] }) }) }
