...
export interface PayrollStructure { id: number; code: string; name: string; version: number; status: string; effective_from: string; effective_to: string | null; currency: string; checksum: string; components: PayrollComponent[] }
...
export interface ERPAccountOption { id: number; code: string; name: string; account_type: string; parent_id: number | null; is_group: boolean }
...
export function useCreatePayrollRun() { const qc = useQueryClient(); return useMutation({ mutationFn: (input: { run_type: PayrollRun['run_type']; period_start: string; period_end: string; tax_point_date: string; statutory_profile_id?: number; employee_ids?: number[]; input_overrides?: Record<string, Record<string, unknown>>; variable_inputs?: PayrollVariableInput[] }) => api.post('/v1/erp/payroll/runs', input).then((r) => r.data as PayrollRun), onSuccess: () => qc.invalidateQueries({ queryKey: ['v1', 'erp', 'payroll', 'runs'] }) }) }
