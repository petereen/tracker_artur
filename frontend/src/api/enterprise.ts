...
export interface PayrollSalaryComponentMaster { id: number; code: string; name: string; component_kind: 'earning' | 'deduction' | 'employer_cost'; formula: string; proration_basis: 'none' | 'working_days' | 'calendar_days' | 'hours'; is_taxable: boolean; is_shi_subject: boolean; is_non_taxable_allowance: boolean; is_leave_average_eligible: boolean; is_flexible_benefit: boolean; max_benefit_amount_yearly?: string; pay_against_benefit_claim?: boolean; only_tax_impact?: boolean; payer: 'employee' | 'employer'; account_id: number | null; cost_center_id: number | null; status: 'draft' | 'active' | 'archived'; source_component_id?: number | null; metadata_json?: Record<string, unknown> }
...
export interface PayrollCapabilities { capabilities: Record<'view_salary' | 'edit_setup' | 'calculate' | 'approve' | 'post' | 'pay' | 'release_slips', boolean> }
...
export function usePayrollRuns(enabled = true) { return useQuery<PayrollRun[]>({ queryKey: ['v1', 'erp', 'payroll', 'runs'], queryFn: () => api.get('/v1/erp/payroll/runs').then((r) => r.data), enabled, retry: false }) }
...
export function useCreatePayrollRun() { const qc = useQueryClient(); return useMutation({ mutationFn: (input: { run_type: PayrollRun['run_type']; period_start: string; period_end: string; posting_date?: string; tax_point_date: string; statutory_profile_id?: number; employee_ids?: number[]; attendance_policy?: Record<string, unknown>; cost_center_id?: number; input_overrides?: Record<string, Record<string, unknown>>; variable_inputs?: PayrollVariableInput[] }) => api.post('/v1/erp/payroll/runs', input).then((r) => r.data as PayrollRun), onSuccess: () => qc.invalidateQueries({ queryKey: ['v1', 'erp', 'payroll', 'runs'] }) }) }
