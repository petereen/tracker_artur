...
test('authenticated task collaboration is reachable with keyboard controls', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Даалгаврын самбар', level: 1 })).toBeVisible()
  await page.locator('.task-card-body').filter({ hasText: 'Release readiness' }).focus()
  await page.keyboard.press('Enter')
  await expect(page.getByRole('tablist', { name: 'Даалгаврын дэлгэрэнгүй' })).toBeVisible()
  await page.getByRole('tab', { name: /Checklist/ }).focus()
  await page.keyboard.press('Enter')
  await expect(page.getByPlaceholder('Checklist-д ажил нэмэх')).toBeVisible()
})
